...
**Sandbox mode (`--mode sandbox`, since v0.74.0).** A fourth mode for **full `$HOME` isolation**, distinct from `local` (which only avoids the `$HOME` *file* but still runs the agent under the real `$HOME`, so a pre-existing global install co-loads). `sandbox` installs GSE under a project-internal `$HOME` at `<project>/.gse-sandbox/` and drops a generic launcher at `<project>/.gse/run` (+ metadata `<project>/.gse/launch.env`). **Install and run are decoupled**: install once, then `sh .gse/run` over many days (no reinstall). Implementation: `install_sandbox()` **re-execs** `install.py --mode plugin` with `HOME=<project>/.gse-sandbox` and `XDG_CONFIG_HOME` unset (so the normal plugin install lands entirely inside the sandbox — no `install.py` path-logic change), then writes the launcher. The launcher (generated as `plugin/gse-run`, copied to `.gse/run`) reads `.gse/launch.env`, and in sandbox mode exports `HOME=<project>/.gse-sandbox` (+ unsets `XDG_CONFIG_HOME`) before `exec`-ing the agent. Supported for **codex / gemini / opencode** only (file-based, `$HOME`-honoring installs); **Claude** (CLI/Keychain-mediated) and **Cursor** (GUI app) are rejected with a pointer to `--mode local`. The re-exec'd child runs with `GSE_CHILD=1` so it suppresses its banner/summary. Single user entry point preserved: `install.sh` forwards `GSE_MODE=sandbox` to `install.py` unchanged.
