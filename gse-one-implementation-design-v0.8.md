# GSE-One — Implementation Design Document v0.8

**Version:** 0.8.0  
**Date:** 2026-04-11  
**Status:** Mono-plugin architecture — cross-platform parity  
**Input:** `gse-one-spec-v0.7.md`, `gse-one-cross-inspection-design-v0.6-vs-spec-v0.7.md`, implementation inspection and restructuration pass  
**Supersedes:** `gse-one-implementation-design-v0.7.md`
...
| v0.7 → v0.8 | **Mono-plugin architecture.** `dist/claude/` + `dist/cursor/` merged into single `plugin/` directory. Agents 8→9 (added `gse-orchestrator` for Claude identity). Cursor rules consolidated from 9+3 `.mdc` to 1 single `000-gse-methodology.mdc`. Hooks format: official event-based (`PreToolUse`/`PostToolUse`), two platform-specific files. Settings simplified to `{"agent": "gse-orchestrator"}`. TIME→COMPLEXITY: `stale_sprint_days`→`stale_sprint_sessions`, all calendar-based metrics removed. Methodology parity: orchestrator body = `.mdc` body, generated from same source. Generator rewrite (~400 lines). |
...
Both platforms use event-based hooks with identical logic but different JSON formats. The hook commands are defined as shared constants in the generator and emitted in platform-specific wrappers.

### 7.1 Claude Code: `hooks/hooks.claude.json`

Claude Code uses PascalCase event names (`PreToolUse`, `PostToolUse`) and an explicit `type` field on each hook entry:
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "if echo \"$CLAUDE_TOOL_INPUT\" | grep -q '^git commit'; then branch=$(git branch --show-current 2>/dev/null); if [ \"$branch\" = 'main' ]; then echo 'GUARDRAIL: Direct commit to main detected. Use a feature branch.'; exit 1; fi; fi"
          },
          {
            "type": "command",
            "command": "if echo \"$CLAUDE_TOOL_INPUT\" | grep -q '^git push --force'; then echo 'EMERGENCY GUARDRAIL: Force push detected. This can cause permanent data loss. Aborting.'; exit 1; fi"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "if echo \"$CLAUDE_TOOL_INPUT\" | grep -q '^git push'; then python3 -c \"import yaml; s=yaml.safe_load(open('.gse/status.yaml')); o=s.get('review_findings_open',0); print(f'WARNING: {o} open review findings') if o > 0 else None\" 2>/dev/null || true; fi"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "if echo \"$CLAUDE_TOOL_INPUT\" | grep -q '.gse/'; then echo 'GSE-One state file modified. Run /gse:status to verify consistency.'; fi"
          },
          {
            "type": "command",
            "command": "if echo \"$CLAUDE_TOOL_INPUT\" | grep -q 'docs/sprints/'; then echo 'Sprint artefact saved. Verify YAML frontmatter includes gse.branch field.'; fi"
          }
        ]
      }
    ]
...
| Shell commands | Identical | Identical |
