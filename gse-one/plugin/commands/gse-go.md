...
| No `.gse/` directory AND directory is empty/near-empty (only excluded dirs) | **New project** | **Automatically execute** `/gse:hug` (do NOT just propose it — run it directly). **Per the Activity Execution Fidelity Invariant (orchestrator)**: open the target activity's source file — `$([ -s .gse/registry ] && cat .gse/registry || cat ~/.gse-one)/skills/hug/SKILL.md` on Claude Code or opencode, `$([ -s .gse/registry ] && cat .gse/registry || cat ~/.gse-one)/commands/gse-hug.md` on Cursor — and apply Step 0 literally. Do NOT paraphrase the language question. No preamble, no table, no technical explanation before Step 0. |
...
1. **Validate tool registry** — Run `[ -s .gse/registry ] && cat .gse/registry || cat ~/.gse-one`. If the file is missing or the path it contains does not exist, warn the user: "GSE-One registry not found. Run `python3 install.py` to fix." and skip dashboard generation.
2. **Regenerate `docs/dashboard.html`** — Run `python3 "$([ -s .gse/registry ] && cat .gse/registry || cat ~/.gse-one)/tools/dashboard.py"` to update the dashboard with current state.
