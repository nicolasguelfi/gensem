...
| **Advanced / Expert** | **All remaining questions in one block** (interactive widget only — see "Text-fallback discipline" below for the no-widget case) | Efficient; no hand-holding needed |
...
**Completeness invariant.** Every dimension NOT resolved by Step 1 — Smart Inference MUST be surfaced to the user as an explicit numbered choice before the profile is saved. A "recommended preset" or bundled default is NOT a substitute for asking — a silently-defaulted preference dimension is a defect, not a shortcut (spec §14.1 — Sprint 1 vs Sprint N+1: *"HUG = Full interview (all 13 dimensions)"*). This holds especially for the **consent-bearing dimensions** — #7 Decision involvement, #10 Learning goals, #11 Contextual tips — which shape how autonomously the agent acts and whether it teaches as it works (§P14 — Knowledge Transfer); these are never inferred and never preset-defaulted. *(Rule added after a Codex onboarding without the interactive widget improvised a preset that set contextual tips ON without ever asking — the user never got to choose the pedagogical behavior.)*

**Text-fallback discipline (no interactive widget — e.g. Codex / Gemini CLI, see spec Appendix C — Per-platform session-transcript persistence's sibling era-bound rows on the absent interactive-question widget).** When the agent falls back to text (spec §P4 — Human-in-the-Loop, "Text fallback"), the "all in one block" expert cadence MUST NOT become a single composite message with free-text fields — that maximizes keystrokes, the opposite of the intent. In text fallback, at every expertise level:
- present each finite-option dimension as a **numbered list**, answerable by a single number;
- group **at most ~3 dimensions per message** (even for experts), so the user replies with a short list like `2 4 1`, never a paragraph;
- convert otherwise-free-text dimensions to **numbered inferred guesses + an "other: type…" escape** — e.g. *Domain background* offers the top inferred domains as numbered options, free text only as the explicit escape.

This mirrors `go.md` Step 7 — Intent Capture, which is already keystroke-minimal.

**Interactive mode (preferred):** When the environment provides an interactive question tool (all three supported runtimes do — see orchestrator P4 for the `AskUser` mapping table), use it to present profile questions as clickable choices instead of numbered text lists. Use `multiSelect: true` for dimensions where multiple values apply. Map "Discuss" to the automatic "Other" escape hatch. When interactive tools are unavailable or dimensions require free text (e.g., learning goals, domain background), fall back to the **numbered-list** format (spec §P4 — Human-in-the-Loop, "Text fallback"), following the "Text-fallback discipline" below — not a free-form conversational block.
...
1. **Git repo check** — If no `.git/` directory exists:
   ...
   - If yes: run `git init`, create initial `.gitignore`, **apply the Git Identity Verification preflight** (see below), then create a **foundational commit** on `main`:
     ...
     cat "$([ -s .gse/registry ] && cat .gse/registry || cat ~/.gse-one)/templates/gitignore-additions.txt" >> .gitignore
...
2. **Create `.gse/` directory** — If it does not exist:
   ...
   - Copy `config.yaml` from the template (`$([ -s .gse/registry ] && cat .gse/registry || cat ~/.gse-one)/templates/config.yaml`) with default values. Leave `lifecycle.mode` empty — `/gse:go` will fill it after Complexity Assessment (spec §14.3 — Orchestrator Decision Logic, Step 6 — Complexity Assessment).
   - Copy `status.yaml` from the template (`$([ -s .gse/registry ] && cat .gse/registry || cat ~/.gse-one)/templates/status.yaml`) verbatim. Its canonical initial state is `current_sprint: 0` / `current_phase: LC01` — the pre-first-sprint baseline (PLAN promotes to sprint 1; see go.md Adopt Step 5 — Set baseline). Do NOT hand-author the file with `null` cursor values: a `null` `current_sprint` is the legitimate baseline meaning but must be encoded as `0`, not `null`, so the dashboard and downstream activities read a numeric sprint.
   - Stamp the GSE-One plugin version in `status.yaml.gse_version` by reading it from the unified version file at `$([ -s .gse/registry ] && cat .gse/registry || cat ~/.gse-one)/VERSION` — a single 1-line text file present at the root of every install target (regardless of platform or plugin/no-plugin mode), written by the generator and distributed by `install.py`. This traces which plugin version created the project state — used by the dashboard header display and by future `/gse:upgrade` migration logic (spec §13.4 — Required fields, design §14.3 — Open questions #3). See CLAUDE.md "Tool architecture" for the registry model that makes this path uniformly resolvable.
...
1. **Validate tool access** — Run: `[ -s .gse/registry ] && cat .gse/registry || cat ~/.gse-one` to verify the plugin registry exists. If the file is missing, warn the user: "GSE-One registry not found. Run `python3 install.py` again to fix."
2. **Generate the first dashboard** — Run: `python3 "$([ -s .gse/registry ] && cat .gse/registry || cat ~/.gse-one)/tools/dashboard.py"`
...
4. Inform the user:
   ...
   - **For intermediate/advanced users:** "Dashboard generated at `docs/dashboard.html`. Regenerated after each activity. Run `python3 \"$([ -s .gse/registry ] && cat .gse/registry || cat ~/.gse-one)/tools/dashboard.py\" --watch` for live updates."
