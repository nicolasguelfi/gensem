...
1. Update `status.yaml` **session state only**:
   ...
   - **Integrity-counter backstop** — run `python3 "$([ -s .gse/registry ] && cat .gse/registry || cat ~/.gse-one)/tools/counters.py" health`; if `"stale": true`, emit the same Inform note as `/gse:go` Step 4 — Stale Sprint Detection (P16 integrity counters unmoved across ≥ 5 transitions — re-check spec §P16 counting obligations).
