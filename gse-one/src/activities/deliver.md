...
9.4 Report delivery summary:
   - Tasks delivered
   - Version tagged (if applicable)
   - Branches cleaned
   - Plan snapshot: `docs/sprints/sprint-{NN}/plan-summary.md`
   - Next step: propose `/gse:compound`
9.5 **Regenerate dashboard** — Run `python3 "$(cat ~/.gse-one)/tools/dashboard.py"` to update `docs/dashboard.html` with delivery status and release info.
