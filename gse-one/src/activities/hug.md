...
2. **Create `.gse/` directory** — If it does not exist:
   ...
   - Copy `status.yaml` from the template (`$(cat ~/.gse-one)/templates/status.yaml`) verbatim. Its canonical initial state is `current_sprint: 0` / `current_phase: LC01` — the pre-first-sprint baseline (PLAN promotes to sprint 1; see go.md Adopt Step 5 — Set baseline). Do NOT hand-author the file with `null` cursor values: a `null` `current_sprint` is the legitimate baseline meaning but must be encoded as `0`, not `null`, so the dashboard and downstream activities read a numeric sprint.
   - Stamp the GSE-One plugin version in `status.yaml.gse_version` by reading it from the unified version file at `$(cat ~/.gse-one)/VERSION` — a single 1-line text file present at the root of every install target (regardless of platform or plugin/no-plugin mode), written by the generator and distributed by `install.py`. This traces which plugin version created the project state — used by the dashboard header display and by future `/gse:upgrade` migration logic (spec §13.4 — Required fields, design §14.3 — Open questions #3). See CLAUDE.md "Tool architecture" for the registry model that makes this path uniformly resolvable.
